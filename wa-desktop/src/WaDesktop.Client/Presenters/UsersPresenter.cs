...
namespace WaDesktop.Client.Presenters
{
    ...
    {
        private readonly IManagementView<User> _view;
        private readonly IApiClient _api;
        private readonly IEventAggregator _bus;
        private bool _disposed;

        public UsersPresenter(IManagementView<User> view, IApiClient api, IEventAggregator bus)
        {
            _view = view;
            _api = api;
            _bus = bus;
        ...
        private void OnAdd(object sender, EventArgs e)
            => MessageBox.Show("Add User — implement form dialog.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);

        private void OnEdit(object sender, EventArgs e)
        {
            if (_view.SelectedIndex < 0) { MessageBox.Show("Pilih baris dulu.", "Info"); return; }
            MessageBox.Show("Edit User — implement form dialog.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void OnDelete(object sender, EventArgs e)
        {
            if (_view.SelectedIndex < 0) { MessageBox.Show("Pilih baris dulu.", "Info"); return; }
            var confirm = MessageBox.Show("Hapus user ini?", "Konfirmasi", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (confirm == DialogResult.Yes)
                MessageBox.Show("Delete User — implement API call.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
